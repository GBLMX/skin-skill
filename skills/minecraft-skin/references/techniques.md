...
> **方向前提**：Java 原版只认 64×64；128 是 OptiFine HD（需 OptiFine/CustomSkinLoader
> 才显示）。要「Java 版可用」应以 **64 原生**设计（`Skin(size=64)`），128 作可选升级。
> 别反着来——128 原生再 BOX 下采样回 64，原版不认且白丢细节。
...
3. **固定输出集**：128 主皮肤 + 64 兼容（BOX）+ 512 放大（NEAREST）
